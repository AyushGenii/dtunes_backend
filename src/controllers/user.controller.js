...
const updateProfileDetails = asyncHandler(async (req, res) => {
  const { username, email, bio, urls, genre, artistBio } = req.body;
  ...
  if (user.role === "artist") {
    ...
    user.artistBio = artistBio || user.artistBio;
  }

  await user.save();

  const updatedUser = await User.findById(user._id).select("username email bio urls role genre artistBio -_id");
